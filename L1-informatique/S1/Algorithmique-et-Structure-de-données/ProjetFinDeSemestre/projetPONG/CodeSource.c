...
typedef enum EcranDeJeu {Accueil , MenuCouleur , EasterEgg , Niveau,  Gameplay, Pause , fin } EcranDeJeu;
            ...
            switch (EcranActuelle) {
                ...
                case Accueil:{
                    // activer l'easter egg 
                     if(IsKeyPressed(KEY_F6)){ //si l'utilisateur appuie sur F6 il vera les credis 
                            EcranActuelle = EasterEgg ;
                       }
                      
                      //continuer 
                      if(IsKeyPressed(KEY_ENTER)) { // faudra appuier sur entrer pour continuer 
                ...
                case EasterEgg :{
                    ...
                    if(IsKeyPressed(KEY_F6)){ // appuiyer sur F6 
                ...
                //ici le joueur devra choisir la couleur du terrain 
                 case MenuCouleur :{
                    if(IsKeyPressed(KEY_B)){  // B poue noir 
                        Couleur = 0;
                        EcranActuelle = Niveau ;
                      }

                     if(IsKeyPressed(KEY_T)){  // T pour blue
                         Couleur = 1;
                         EcranActuelle = Niveau ;
                      } 

                     if(IsKeyPressed(KEY_R)){    // R pour rouge
                         Couleur = 2 ;
                         EcranActuelle = Niveau ;
                      }
                  }break;
                 ...
                 case Niveau :{
                     if(IsKeyPressed(KEY_D)){     //D pour facile 
                         difficulte = 1 ;
                         EcranActuelle =  Gameplay ;        
                      }

                     if (IsKeyPressed(KEY_I)){    //I pour moyen
                         difficulte = 2 ;
                         VitesseRaquetteD = 400 ;
                         EcranActuelle =  Gameplay ;  
                      }

                     if(IsKeyPressed(KEY_E)){     // E pour difficile 
                         difficulte = 3 ;
                 ...
                 case Pause : {
                            if(IsKeyPressed(KEY_F1)){    //F1 pour quitter la mode pause 
                               EcranActuelle = Gameplay ;
                             }
                  ...
                  case fin :{
                         if(IsKeyPressed(KEY_SPACE)) {     // espace pour rejouer 
                               EcranActuelle = Gameplay;
                          }else{
                             if(IsKeyPressed(KEY_TAB)){      //TAB pour revenir a l'accueil 
                                 EcranActuelle = Accueil ;
                              }
                           }
                         }
           ...
           switch ( EcranActuelle )
            {
               ...
               case Accueil : {
                ...
                case fin : {
                       if(gagnant == 2){  // ceci est l'ecran de win pour le jouer
                           EcranGagnant(Couleur);
                          }

                       if(gagnant == 1){  //ceci est l'ecran de win pour le bot 
                           EcranLose(Couleur);
                          }
